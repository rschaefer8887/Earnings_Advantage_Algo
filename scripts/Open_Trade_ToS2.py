""""
Open_Trade_ToS2 — Send equity entry trades to Schwab (secondary ToS account) from Live_Trade_Info.xlsx.
...
- Schwab_Auth.py configured and run at least once to complete OAuth:
    ...
    - Set account_id2 to the secondary account hash from get_account_numbers (see Schwab API docs)
"""
...
from price_run_timestamps import write_open_trade_tos2_success
...
def main() -> None:
    ...
    if not account_id:
        print("account_id2 is missing from schwab_config.json; cannot place orders.")
        return

    print(f"\nPlacing orders to Schwab account {account_id} (secondary) ...")
    submitted = 0
    ...
    if submitted > 0:
        write_open_trade_tos2_success(trade_count=len(trades))
    else:
        print("No orders submitted successfully; open-trade ToS2 success stamp not written.")
